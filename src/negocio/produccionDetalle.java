...
public class produccionDetalle extends Conexion {
  ...
  private double sub_total;
    ...
    public ResultSet configurarTablaDetalleCompra() throws Exception {
        String sql = "select * from( select 0 as codigo_producto, ''::character varying(100) producto, 0 as peso_producto,0 as cantidad_sacos,0 as cantidad_kilos , 0 as precio_pilado,0 as importe_pilado,0 as codigo_envase,''::character varying(100) envase,0 cantidad_envases_uti,0 as precio_envase,0 importe_envases) as tb_temporal where tb_temporal.codigo_producto <> 0";
        ResultSet resultado = this.ejecutarSQLSelect(sql);
        return resultado;
    }
    
    public ResultSet configurarTablaCascara() throws Exception {
        String sql = "select * from( select 0 as codigo, ''::character varying(100) variedad, 0 as cantidad,0 as porcentaje_humendad) as tb_temporal where tb_temporal.codigo <> 0";
        ResultSet resultado = this.ejecutarSQLSelect(sql);
        return resultado;
    }
    
    public ResultSet configurarTablaInsumo() throws Exception {
        String sql = "select * from( select 0 as codigo, ''::character varying(100) insumo, 0 as cantidad,0 as precio,0 as importe) as tb_temporal where tb_temporal.codigo <> 0";
        ResultSet resultado = this.ejecutarSQLSelect(sql);
        return resultado;
    }
